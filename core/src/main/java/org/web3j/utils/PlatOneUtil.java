...
import org.web3j.abi.PlatOneTypeDecoder;
...
public class PlatOneUtil {
    ...
    public static String invokeEncode(Function function,long txType) {
        ...
        result.add(RlpString.create(Numeric.hexStringToByteArray(PlatOneTypeEncoder.encode(new Int64(txType)))));
        result.add(RlpString.create(Numeric.hexStringToByteArray(PlatOneTypeEncoder.encode(new Utf8String(function.getName())))));

        List<Type> parameters = function.getInputParameters();
        for (Type parameter:parameters) {
            String encodedValue = PlatOneTypeEncoder.encode(parameter);
     ...
     * @return encoded data
     */
    public static String invokeEncode(String cnsName,Function function,long txType) {
        List<RlpType> result = new ArrayList<>();
        result.add(RlpString.create(Numeric.hexStringToByteArray(PlatOneTypeEncoder.encode(new Int64(txType)))));
        result.add(RlpString.create(Numeric.hexStringToByteArray(PlatOneTypeEncoder.encode(new Utf8String(cnsName)))));
        result.add(RlpString.create(Numeric.hexStringToByteArray(PlatOneTypeEncoder.encode(new Utf8String(function.getName())))));
    ...
    public static String deployEncode(String contractBinary,String abi) {
        // txType + bin + abi
        List<RlpType> result = new ArrayList<>();
        result.add(RlpString.create(Numeric.hexStringToByteArray(PlatOneTypeEncoder.encode(new Int64(1)))));
        result.add(RlpString.create(Numeric.hexStringToByteArray(contractBinary)));
        result.add(RlpString.create(Numeric.hexStringToByteArray(PlatOneTypeEncoder.encode(new Utf8String(abi)))));
        String data = Hex.toHexString(RlpEncoder.encode(new RlpList(result)));
        return data;
    }
    ...
    public static List<Type> eventDecode(String data, List<TypeReference<Type>> outputParameters) {
        ...
        for (int i = 0; i < outputParameters.size(); i++) {
            ...
            Type result = PlatOneTypeDecoder.decode(rlpBytes, type);
            results.add(result);
        }
        return results;
    }
