...
import org.web3j.abi.PlatOneEventEncoder;
...
public abstract class PlatOneContract extends ManagedTransaction {
    ...
    protected PlatOneContract(String contractBinary, String contractAddress,
    ...
    protected PlatOneContract(String contractAddress,
                              Web3j web3j, Credentials credentials) {
    ...
    private List<Type> executeCall(Function function) throws IOException {
        ...
        if (isCNS){
            encodedFunction = PlatOneUtil.invokeEncode(this.cnsName, function, 17);
            org.web3j.protocol.core.methods.response.EthCall ethCall = web3j.ethCall(
                    Transaction.createEthCallTransaction(
                            transactionManager.getFromAddress(), contractAddress, encodedFunction, BigInteger.valueOf(17)),
                    defaultBlockParameter)
                    .send();

            value = ethCall.getValue();
        }else{
            encodedFunction = PlatOneUtil.invokeEncode(function,getTransactionType(function));
            org.web3j.protocol.core.methods.response.EthCall ethCall = web3j.ethCall(
                    Transaction.createEthCallTransaction(
    ...
    private TransactionReceipt executeTransaction(Function function, BigInteger weiValue) throws IOException, TransactionException {
        ...
        if (isCNS){
            invokeData = PlatOneUtil.invokeEncode(this.cnsName,function,17);
            return executeTransaction(invokeData , weiValue, function.getName(),BigInteger.valueOf(17));
        }else{
            invokeData = PlatOneUtil.invokeEncode(function,getTransactionType(function));
        }
    ...
    private TransactionReceipt executeTransactionWithNonce(Function function, BigInteger nonce) throws IOException, TransactionException {
        ...
        if (isCNS){
            invokeData = PlatOneUtil.invokeEncode(this.cnsName,function,17);
            return executeTransactionWithNonce(invokeData , nonce, function.getName(),BigInteger.valueOf(17));
        }else{
            invokeData = PlatOneUtil.invokeEncode(function,getTransactionType(function));
        }
    ...
    TransactionReceipt executeTransaction(
            String data, BigInteger weiValue, String funcName, BigInteger txType)
            throws TransactionException, IOException {
    ...
    TransactionReceipt executeTransactionWithNonce(
            String data, BigInteger nonce, String funcName,BigInteger txType)
            throws TransactionException, IOException {
    ...
    private static <T extends PlatOneContract> T create(
            T contract, String binary, String abi, String encodedConstructor, BigInteger value)
            throws IOException, TransactionException {

        String data = PlatOneUtil.deployEncode(binary, abi);
    ...
    protected static <T extends PlatOneContract> T deploy(
            Class<T> type,
            Web3j web3j, Credentials credentials,
    ...
    public static <T extends PlatOneContract> RemoteCall<T> deployRemoteCall(
            Class<T> type,
            Web3j web3j, Credentials credentials,
            BigInteger gasPrice, BigInteger gasLimit,
            String binary, String abi, String encodedConstructor, BigInteger value) {
    ...
    public static EventValues staticExtractEventParameters(
        ...
        String encodedEventSignature = PlatOneEventEncoder.encode(event);
        if (!topics.get(0).equals(encodedEventSignature)) {
            return null;
        }

        List<Type> indexedValues = new ArrayList<>();
        List<Type> nonIndexedValues = PlatOneUtil.eventDecode(log.getData(), event.getNonIndexedParameters());
